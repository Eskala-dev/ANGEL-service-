...
export const verifyAdminRole = async (req, res, next) => {
  try {
    const { idUser } = req.body; // Asegúrate de que el ID del usuario se pase en el cuerpo de la solicitud
    ...
    // Si el rol es "admin", continuar con la siguiente función
    next();
  } catch (error) {
...
router.post('/send-cod-reset-password', async (req, res) => {
  try {
    ...
    if (usuarioEncontrado) {
      ...
      res
        .status(403)
        .send(`${filtro === 'usuario' ? 'Este usuario no existe' : 'No existe usuario con este correo electrónico'}`);
    }
  } catch (error) {
    console.error(error);
...
router.post('/register', checkUniqueFields, async (req, res) => {
  ...
  try {
    ...
    emitToClients('server:onNewUser', usuarioGuardado, socketId);
...
router.put('/edit-user/:id', checkUniqueFields, async (req, res) => {
  ...
  try {
    ...
    emitToClients('server:onChangeUser', updatedUser._id, socketId);
    emitToClients('server:onUpdateUser', updatedUser, socketId);
...
router.put('/delete-user/:id', async (req, res) => {
  ...
  try {
    ...
    const user = await Usuario.findByIdAndUpdate(userId, { state: 'eliminado' }, { new: true, select: '_id' });
    ...
    const socketId = req.headers['x-socket-id'];
    emitToClients('server:onDeleteUser', user._id, socketId);
    emitToClients('server:onDeleteAccount', user, socketId);
