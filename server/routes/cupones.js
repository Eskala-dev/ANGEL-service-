...
router.post('/generate-multiples-cupones', async (req, res) => {
  try {
    ...
    if (!codigoPromocion || !cantCupones || isNaN(cantCupones) || cantCupones <= 0) {
      return res.status(400).json({
        mensaje: 'Debe proporcionar un código de promoción válido y una cantidad válida de cupones mayor a cero',
      });
    }
    ...
    while (codigosGenerados.length < cantCupones) {
      ...
      const nuevosCodigos = generateUniqueCodes(cantidadRestante, codigosGenerados);
...
function generateUniqueCodes(amount, existingCodes) {
  ...
  while (uniqueCodes.length < amount) {
    ...
    } while (existingCodes.includes(codigoCupon) || uniqueCodes.includes(codigoCupon));
      ...
      if (!promocion) {
        ...
        return res.status(200).json({
          ...
          promocion: {
            ...
            seletionServices: promocion.seletionServices,
