...
router.get('/get-reporte-anulados/:date', async (req, res) => {
  try {
    ...
    const idsUserOnAnulados = [...new Set(infoAnulados.map((anulado) => anulado.idUser))];

    // Realizar una sola consulta en la colección de Usuarios utilizando los idUser obtenidos
    const usuarios = await Usuarios.find({ _id: { $in: idsUserOnAnulados } }, { name: 1, rol: 1 });

    // Obtener las facturas asociadas a los reportes anulados
    const facturas = await Factura.find(
      { _id: { $in: ordenesAnuladosIds } },
      { codRecibo: 1, Nombre: 1, totalNeto: 1, infoRecepcion: 1 }
    ).lean();
    ...
    const reportesDetallados = infoAnulados.map((anulado) => {
      ...
      return {
        ...
        infoRecepcion: factura.infoRecepcion,
        fechaAnulacion: {
          fecha: anulado.fecha,
          hora: anulado.hora,
        },
