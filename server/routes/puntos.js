...
import { verifyAdminRole } from './usuarios.js';
import { emitToClients } from '../socket/socketServer.js';

const router = express.Router();

export const checkPuntosState = async () => {
  try {
    const puntosConfig = await modelPuntos.findOne({}, { useState: 1 });
    return puntosConfig ? puntosConfig.useState === true : false;
  } catch (error) {
    return false;
  }
};
...
router.post('/reset-info-score', verifyAdminRole, async (req, res) => {
  try {
    await clientes.updateMany({}, { $set: { infoScore: [], scoreTotal: 0 } });

    emitToClients('server:resetPuntos', 'Puntaje de Cliente Resetado Correctamente');

    res.json({
      mensaje: 'Puntaje de Cliente Resetado Correctamente',
    });
  } catch (error) {
    console.error('Error al actualizar infoScore:', error);
    res.status(500).json({ mensaje: 'Error al actualizar infoScore' });
  }
});
